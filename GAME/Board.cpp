    ...
    for (int i = 0; i < NUM_TILES; ++i) {
        ...
        if(board[i].getTileType() == TileType::PROPERTY) {
            ...
            case ColorGroup::NAVAJO_WHITE:
                SDL_SetRenderDrawColor(gameInstance->getRenderer(), 255, 222, 173, 255); // navajo trắng
                break;
            default:
            ...
            case TileType::LOST_ISLAND:
                SDL_SetRenderDrawColor(gameInstance->getRenderer(), 0, 206, 209, 255);  // Xanh lá
                break;
            case TileType::WORLD_TOUR:
                SDL_SetRenderDrawColor(gameInstance->getRenderer(), 128, 128, 128, 255);  // Màu xám
                break;
            case TileType::WORLDS:
                SDL_SetRenderDrawColor(gameInstance->getRenderer(), 100, 149, 237, 255);  // màu xanh hoa ngô đồng
                break;
            ...
            case TileType::BEACH:
                ...
                break;
            case TileType::TAX:
                SDL_SetRenderDrawColor(gameInstance->getRenderer(), 245, 255, 250, 255);  // kem bạc hà
                break;
            default:
